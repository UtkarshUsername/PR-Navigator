...
export function GraphNode({ data, selected }: NodeProps<FlowBoardNode>) {
  ...
  return (
    <div className={`nav-node nav-node--${data.kind} ${selected ? 'is-selected' : ''}`}>
      ...
      <div className="nav-node__surface">
      <div className="nav-node__header">
        ...
        <div className="nav-node__badges">
          {data.isOwnedByMe ? <span className="nav-node__badge nav-node__badge--mine">By me</span> : null}
        ...
        <a
          className="nav-node__view-link nodrag nopan"
          href={data.githubUrl}
          target="_blank"
          rel="noopener noreferrer"
          aria-label={`Open ${nodeLabel} #${data.number} on GitHub`}
        >
          View -&gt;
        </a>
      </div>
